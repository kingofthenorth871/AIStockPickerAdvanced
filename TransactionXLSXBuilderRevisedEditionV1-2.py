        ...
        for ticker in tickers:
            ...
            if open_price is None:
                ...
                try:
                    ...
                    twentitwentione = datetime(2019, 1, 1)
                    daysSinceStartTime200 = datetime(2019, 1, 1) - timedelta(days=200)
                    ...
                    stopTradingDate = datetime(2020, 1, 2)
                    stopTrading = False
                    ...
                    for date in DatesToIterateOverAllPrices:
                        ...
                        if (fiftyDayMovingAverage<twoHoundredDayMovingAverage and buyOrSell!='Sell' and buyOrSell!=None or stopTrading==True and buyOrSell!='Sell'):
                            print('Sell')
                        ...
                        if (fiftyDayMovingAverage>twoHoundredDayMovingAverage and buyOrSell!='Buy' and stopTrading==False):
                            print('Buy')
                            print('50 day moving average')
                            print(fiftyDayMovingAverage)
                            print('200 day moving average')
...
def mainTransactionBuilder(year):
    ...
    tickers = pd.read_excel(fileString4)
    tickers = tickers['tickers'].values.tolist()

    #del tickers[6]
    print(tickers)

    contentString = "datatickers123-2"
    contentString2 = year
    contentString3 = ".xlsx"
    contentString4 = contentString + contentString2 + contentString3

    df = pull_stock_data(tickers)
    df.to_excel(contentString4)
...
async def foo1():
    mainTransactionBuilder("2015")
    await asyncio.sleep(0.1)

async def foo2():
    mainTransactionBuilder("2016")
    await asyncio.sleep(0.1)

async def foo3():
    mainTransactionBuilder("2017")
    await asyncio.sleep(0.1)

async def foo4():
    mainTransactionBuilder("2018")
    await asyncio.sleep(0.1)
