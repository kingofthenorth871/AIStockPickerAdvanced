...
def trainClassifierAndPickOutBestStocks(probability, Xval, yval, data):
    ...
    data2019or = pd.read_csv("SelfMadeStockDatasetCleanedInStockPicker2021.csv")

    data2019unclean = pd.read_csv("SelfMadeStockDataset2021.csv")
    ...
    data2018or = pd.read_csv("SelfMadeStockDatasetCleanedInStockPicker2021.csv")
   ...
   # print("prediction: ")
    ...
    winnerStockDataFrame = pickOutWinnersBasedOnProbabilityFromClassifierPrediction(0.999999, classifier, columns, data2019or)
...
data = pd.read_csv("SelfMadeStockDataset2019.csv")
...
conditions = [
    mod_data['priceVar1yr'] > 550 , mod_data['priceVar1yr'] < 550
    #mod_data['priceVar1yr'] < -95 , mod_data['priceVar1yr'] > -95 #short version
]
...
mod_data.to_csv('SelfMadeStockDatasetCleanedInStockPicker2019.csv')
...
#finalDataFrameList = pd.DataFrame({"tickers":[],
 #                   "class":[]})
...
while len(allTickers)<150:
    winnerStockDataFrame = trainClassifierAndPickOutBestStocks(0.999999, X, y, mod_data)
    print('size of winnerStockDataFrame')
    print(len(winnerStockDataFrame))
    lengthOfDataframe=len(winnerStockDataFrame)
    tickers = winnerStockDataFrame['tickers'].values.tolist()

    if 0 < len(winnerStockDataFrame) < 70:
...
finalDataFrameList.to_excel('stockWinnersFromAIStockPicker2.xlsx')
